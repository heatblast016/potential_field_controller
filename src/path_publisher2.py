...
def run_plan(pub_init_pose, pub_controls, plan):
    ...
    for c in plan:
        ...
        print(c)
        while(distance_to_target(targ_x, targ_y) > 0.75):
...
def laserVector(dist, angle):
    if(math.isnan(dist)):
        #handles faulty sensor measurements
        dist = 100000000
    mag = kL/dist
...
def send_command(pub_controls, c):
    ...
    #replace with calculateMulti for multi-agent
...
# subscribe to particle filter
    ...
    sub_pf = rospy.Subscriber("/car2/car_pose" , PoseStamped, callbackpose, queue_size=10)
    sub_ls = rospy.Subscriber("/car2/scan", LaserScan, callbackscan)
    sub_oc = rospy.Subscriber("/car1/car_pose" , PoseStamped, callbackposeoc, queue_size=10)
    control_topic = rospy.get_param("~control_topic", "/car2/mux/ackermann_cmd_mux/input/navigation")
